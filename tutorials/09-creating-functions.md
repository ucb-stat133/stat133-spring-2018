...
R has the function `scale()` that can be used to perform this operation, but let's pretend for a minute that there's no function in R to calculate standard scores.
...
Now `standardize()` is able to return a more useful output by removing missing values. However, we should let the user decide is `NA`s must be removed. We can include an argument in `standardize()` to indicate if missing values are to be removed:
...
Having simplified the code, we can now modify our function:
...
-   Roxygen comments are R comments formed by the hash immediately followed by an apostrophe: `#'`

-   You specify a field with `@` and a keyword: e.g. `@title`
