...
Although R has built-in functions to perform regex operations, I've found that
...
[https://raw.githubusercontent.com/ucb-stat133/stat133-spring-2018/master/data/may-logs.txt](https://raw.githubusercontent.com/ucb-stat133/stat133-spring-2018/master/data/may-logs.txt)
...
Here's an example of a log record (the text should in one line of code, but
...
contain the pattern `"jpg"`. We can use `grep()` to match or detect this pattern:
...
Another common task when working with regular expressions has to do with pattern 
extraction. For this purposes, we can use `str_extract()`:
...
str_extract(sublogs, "\\w+\\.(jpg|png|gif|ico)")
```

- How to get just the name of the image file without, the extension (`image`)?

```{r echo = FALSE, results = 'hide'}
str_sub(str_extract(sublogs, "\\w+\\.(jpg|png|gif|ico)"), end = -5)
```

- How to get the request type: e.g. `"GET`?

```{r echo = FALSE, results = 'hide'}
str_extract(sublogs, '\\"[A-Z]+')
```

- How to get the status codes: e.g. `200`?

```{r echo = FALSE, results = 'hide'}
str_extract(sublogs, '\\" [0-9][0-9][0-9]')
str_extract(sublogs, ' [0-9][0-9][0-9]')
```

- How to get the size of the resource (number at the end): e.g. `34301"`?

```{r echo = FALSE, results = 'hide'}
str_extract(sublogs, ' [0-9]+$')
str_extract(sublogs, '[0-9]+$')
```
...
```{r echo = FALSE, results = 'hide'}
# fail
str_extract(sublogs, "\\w+")
str_extract(sublogs, "\\w+\\.+ ")
str_extract(sublogs, "(\\w+\\.\\w+)+")

# almost (fails at matching 'pd9049f35.dip.t-dialin.net')
str_extract(sublogs, "(\\w+\\.)+\\w+")

# okay
str_extract(sublogs, "(\\w+\\.)+\\w+-*(\\w+\\.)+\\w+")
```
